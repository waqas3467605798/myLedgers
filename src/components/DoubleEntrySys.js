...
import {Link, Route, BrowserRouter} from 'react-router-dom'
  ...
  class DoubleEntrySys extends Component{
    constructor(){
      super();
      this.state ={
              user:null,
...
<h2 style={{textAlign:'center'}} className='headings'>Double Entry Accounting System</h2>
          <div>
           <MainBar />
          <Route exact path='/DoubleEntrySys' component={DataEntry} />
          <Route path='/DoubleEntrySys/Content' component={Content}/>
          <Route path='/DoubleEntrySys/Ledger' component={Ledger}/>
          <Route path='/DoubleEntrySys/Trial' component={Trial}/>
          <Route path='/DoubleEntrySys/Vouchers' component={Vouchers}/>
          </div>
    ...
    constructor(){
      ...
      render(){
        return(
          ...
          <Link to='/DoubleEntrySys/Content' style={{textDecoration:'none', marginRight:'22px', color:'red'}}> Create Account</Link>
          <Link to='/DoubleEntrySys' style={{textDecoration:'none', marginRight:'22px', color:'red'}}> Data Entry </Link>
          <Link to='/DoubleEntrySys/Vouchers' style={{textDecoration:'none', marginRight:'22px', color:'red'}} > Vouchers </Link>
          <Link to='/DoubleEntrySys/Ledger' style={{textDecoration:'none', marginRight:'22px', color:'red'}} > Account Statement </Link>
          <Link to='/DoubleEntrySys/Trial' style={{textDecoration:'none', marginRight:'22px', color:'red'}} > Summary </Link>
  ...
  class Content extends Component{
    ...
    dataPushPromise.then(()=>{
      ...
      var pushPromise = new Promise((res,rej)=>{
        var obj = [];
        firebase.database().ref('partyList_DoubEntry'+this.state.user).on('child_added' , (data)=> { 
          obj.push(data.val())
        }  )
        res(obj);
...
var key = firebase.database().ref('partyList_DoubEntry'+this.state.user).push().key
partyObj.key = key
firebase.database().ref('partyList_DoubEntry'+this.state.user).child(key).set(partyObj)
...
firebase.database().ref('partyList_DoubEntry'+this.state.user).child(reqObj.key).set(reqObj)
...
class DataEntry extends Component{
    ...
    dataPushPromise.then(()=>{
      var pushPromise = new Promise((res,rej)=>{
        ...
        firebase.database().ref('VoucherNumber_DoubleEntry'+this.state.user).on('child_added' , (data)=> {
...
firebase.database().ref('VoucherNumber_DoubleEntry'+this.state.user).child('VoucherNumber').set(vouNum)
...
class Ledger extends Component{
  ...
  accountDelete = ()=>{
    ...
    firebase.database().ref('partyList_DoubEntry'+this.state.user).child(key).remove()
    //code ended
  ...
  <div className={this.state.accountDeleteRefresh === false ? 'display' : ''} style={{textAlign:'center'}}>
    ...
    <Link to='/DoubleEntrySys/Trial' onClick={this.accountDelRfrsh}> <button className="waves-effect waves-dark btn"> OK </button></Link>
  </div>
  ...
  <div className={this.state.cancelDelete === false ? 'display' : ''} style={{textAlign:'center'}}>
    ...
    <Link to='/DoubleEntrySys/Ledger' onClick={this.cancelDelRfrsh}> <button className="waves-effect waves-dark btn"> OK </button></Link>
  </div>
